...
partial class ListQuery
{
    ...
    /// if <paramref name="index"/> is out of range, return <paramref name="defaultValue"/>
    /// </summary>
    public static T AtOrDefaultROList<T>(this IReadOnlyList<T> list, int index, T defaultValue = default!) 
        => index >= list.Count || index < 0 ? defaultValue : list[index];
    ...
    /// if <paramref name="index"/> is out of range, return <paramref name="defaultValue"/>
    /// </summary>
    public static T AtOrDefaultROList<T>(this IReadOnlyList<T> list, Index index, T defaultValue = default!)
        => index.Value >= list.Count ? defaultValue : list[index];
}
