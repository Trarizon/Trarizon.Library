...
public static partial class TraCollection
{
    ...
    public readonly struct ListSortedModifier<T, TComparer> where TComparer : IComparer<T>
    {
        ...
        public int BinarySearch(Range priorRange, T item)
            => TraSpan.BinarySearchRangePriority(_list.AsSpan(), priorRange, new ComparerComparable<T, TComparer>(item, _comparer));
        ...
        public int LinearSearch(Index nearIndex, T item)
            => TraSpan.LinearSearchFromNear(_list.AsSpan(), nearIndex.GetOffset(Count), new ComparerComparable<T, TComparer>(item, _comparer));
