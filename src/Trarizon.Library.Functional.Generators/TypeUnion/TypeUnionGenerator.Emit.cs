...
partial class TypeUnionGenerator
{
    ...
    {
        foreach (var variant in data.Variants.Where(x => x.TypeKind is not VariantTypeKind.Void).JoinWriteEmptyLine(writer))
        {
            var @unsafe = variant.TypeKind.IsPointer() ? "unsafe " : "";
            var cast = variant.TypeKind.IsPointer() ? $"(global::System.IntPtr)" : "";
            writer.WriteLine(Utils.GeneratedCodeAttributeList);
    ...
    private void EmitAsMethod(IndentedTextWriter writer, TypeUnionData data, Env env)
    {
        ...
        using (writer.EnterBracketIndentScope('{'))
        {
            ...
            using (writer.EnterBracketIndentScope('{'))
            {
                foreach (var variant in data.Variants.Where(x => x.TypeKind.IsGenericable()))
                {
                    if (env.AllowsRefStruct || !variant.IsRefLikeType)
                    {
                        writer.WriteLine($"case {variant.Id}u:");
                        using (writer.EnterBracketIndentScope('{'))
                        {
                            writer.WriteLine($"if (typeof(T) == typeof({variant.TypeFQName}))");
                            writer.WriteLine($"    return {ExprVariantToT(variant)};");
                            if (!variant.IsRefLikeType)
                            {
                                writer.WriteLine($"if ({ExprVariantToT(variant, variant.TypeFQName)} is T typed)");
    ...
    private void EmitAsExactlyMethod(IndentedTextWriter writer, TypeUnionData data, Env env)
    {
        ...
        if (!env.AllowsRefStruct)
        {
            ...
            foreach (var variant in data.Variants.Where(x => x.IsRefLikeType))
            {
                ...
                writer.WriteLine(Utils.GeneratedCodeAttributeList);
                writer.WriteMultipleLines($$"""
                    public readonly {{variant.TypeFQName}} As_{{variant.ReadableName}}()
                    {
                        if (this.__um_flag == {{variant.Id}}u)
                            return {{ExprVariantToT(variant, variant.TypeFQName)}};
                        return default({{variant.TypeFQName}});
                    }
                    """);
            }
        }
        ...
        if (data.Variants.Any(x => x.TypeKind is VariantTypeKind.Pointer))
        {
            writer.WriteLine();
            writer.WriteLine(Utils.GeneratedCodeAttributeList);
            writer.WriteLine($"public unsafe readonly T* AsPointer<T>()");
            using (writer.EnterBracketIndentScope('{'))
            {
                writer.WriteLine("switch(this.__um_flag)");
                using (writer.EnterBracketIndentScope('{'))
                {
                    foreach (var variant in data.Variants.Where(x => x.TypeKind is VariantTypeKind.Pointer))
                    {
                        writer.WriteLine($"case {variant.Id}u:");
                        using (writer.EnterBracketIndentScope('{'))
                        {
                            writer.WriteLine($"if (typeof(T*) == typeof({variant.TypeFQName}))");
                            writer.WriteLine($"    return {ExprVariantPtrToT(variant)};");
                            writer.WriteLine("break;");
                        }
                    }
                }
        ...
        if (data.Variants.FirstOrDefault(x => x.TypeKind is VariantTypeKind.VoidPointer) is { } voidPointerVariant)
        {
            writer.WriteLine();
            writer.WriteLine(Utils.GeneratedCodeAttributeList);
            writer.WriteMultipleLines($$"""
                ...
                {
                    if (this.__um_flag == {{voidPointerVariant.Id}}u)
                        return {{ExprVariantPtrToT(voidPointerVariant, "void")}};
                    return default(void*);
                }
                """);
        }
                ...
                foreach (var variant in data.Variants.Where(x => x.TypeKind.IsGenericable()))
                {
                    if (env.AllowsRefStruct || !variant.IsRefLikeType)
                    {
                        ...
                        using (writer.EnterBracketIndentScope('{'))
                        {
                            ...
                            if (!variant.IsRefLikeType)
                            {
                                writer.WriteLine($"return {ExprVariantToT(variant, variant.TypeFQName)} is T;");
                            }
                            else
                            {
    ...
    private void EmitIsExactlyMethod(IndentedTextWriter writer, TypeUnionData data, Env env)
    {
        ...
        if (data.Variants.FirstOrDefault(x => x.TypeKind is VariantTypeKind.Void) is { } voidVariant)
        {
            writer.WriteLine();
            writer.WriteLine(Utils.GeneratedCodeAttributeList);
            writer.WriteMultipleLines($$"""
        ...
        if (!env.AllowsRefStruct)
        {
            foreach (var variant in data.Variants.Where(x => x.IsRefLikeType))
            {
                ...
                writer.WriteLine(Utils.GeneratedCodeAttributeList);
                writer.WriteMultipleLines($$"""
                    public readonly bool Is_{{variant.ReadableName}}()
                    {
                        return this.__um_flag == {{variant.Id}}u;
                    }
                    """);
        ...
        if (data.Variants.Any(x => x.TypeKind is VariantTypeKind.Pointer))
        {
            writer.WriteLine();
            writer.WriteLine(Utils.GeneratedCodeAttributeList);
            writer.WriteLine($"public unsafe readonly bool IsPointer<T>()");
            using (writer.EnterBracketIndentScope('{'))
            {
                foreach (var variant in data.Variants.Where(x => x.TypeKind is VariantTypeKind.Pointer))
                {
        ...
        if (data.Variants.FirstOrDefault(x => x.TypeKind is VariantTypeKind.VoidPointer) is { } voidPointerVariant)
        {
            ...
            writer.WriteMultipleLines($$"""
                ...
                {
                    return this.__um_flag == {{voidPointerVariant.Id}}u;
                }
                """);
        }
    }
    ...
    {
        var @ref = data.Variants.Any(x => x.TypeKind.IsUnmanaged() && x.IsRefLikeType) ? "ref " : "";
        ...
        using (writer.EnterBracketIndentScope('{'))
        {
            foreach (var variant in data.Variants.Where(v => v.TypeKind is VariantTypeKind.Unmanaged))
            {
                writer.WriteLine($"[global::System.Runtime.InteropServices.FieldOffset(0)]");
                writer.WriteLine($"public {variant.TypeFQName} _{variant.FieldId};");
            }
            if (data.Variants.Any(x => x.TypeKind.IsPointer()))
            {
                writer.WriteLine($"[global::System.Runtime.InteropServices.FieldOffset(0)]");
    ...
    private void EmitDangerousGetValueMethods(IndentedTextWriter writer, TypeUnionData data, Env env)
    {
        ...
        // unmanaged
        if (data.Variants.Any(x => x.TypeKind.IsUnmanaged() && (env.AllowsRefStruct || !x.IsRefLikeType)))
        {
            writer.WriteLine(Utils.GeneratedCodeAttributeList);
            writer.WriteLine(Utils.AggressiveInliningAttributeList);
            if (env.UnscopedRef)
        ...
        // managed
        if (data.Variants.Any(x => x.TypeKind is VariantTypeKind.Managed && (env.AllowsRefStruct || !x.IsRefLikeType)))
        {
            writer.WriteLine(Utils.GeneratedCodeAttributeList);
            writer.WriteLine(Utils.AggressiveInliningAttributeList);
            if (env.UnscopedRef)
            ...
            using (writer.EnterBracketIndentScope('{'))
            {
                ...
                {
                    if (env.AllowsRefStruct || !item.IsRefLikeType)
                    {
                        writer.WriteMultipleLines($$"""
        ...
        if (!env.AllowsRefStruct && data.Variants.Any(x => x.IsRefLikeType))
        {
            foreach (var variant in data.Variants.Where(x => x.IsRefLikeType))
            {
                writer.WriteLine(Utils.GeneratedCodeAttributeList);
                ...
                if (env.UnscopedRef)
                    writer.WriteLine(Utils.UnscopedRefAttributeList);
                writer.WriteMultipleLines($$"""
                    private readonly ref readonly {{variant.TypeFQName}} DangerousGetValueRef_{{variant.ReadableName}}()
                    {
                        return ref {{ExprVariantToT(variant, variant.TypeFQName)}};
                    }
                    """);
                writer.WriteLine();
            }
    ...
    private string ExprVariantFieldAccess(VariantData variant, string @this = "this")
    {
        return variant.TypeKind switch
        {
            ...
            VariantTypeKind.VoidPointer => $"{@this}.__um_unmanaged._ptr",
