    ...
    internal ref struct DefaultInterpolatedStringHandler
    {
        ...
        public void AppendFormatted<T>(T value)
        {
            ...
            if (value is IFormattable)
            {
                ...
                if (typeof(T).IsEnum)
                {
                    ...
                    while (_pos + text.Length > _chars.Length)
                    {
                        Grow();
                    }

                    text.AsSpan().TryCopyTo(_chars.Slice(_pos));
                    _pos += text.Length;
        ...
        public void AppendFormatted<T>(T value, string? format)
        {
            ...
            if (value is IFormattable)
            {
                ...
                if (typeof(T).IsEnum)
                {
                    var text = Enum.Format(typeof(T), value, format);
                    while (_pos + text.Length > _chars.Length)
                    {
                        Grow();
                    }
        ...
        private void GrowCore(uint requiredMinCapacity)
        {
            ...
            int arraySize = (int)(newCapacity < MinimumArrayPoolLength ? MinimumArrayPoolLength : newCapacity > int.MaxValue ? int.MaxValue : newCapacity);
